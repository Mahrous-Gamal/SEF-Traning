...
const Header = ({ flag }) => {
  ...
  return (
    <section
      className="header-component position-absolute top-0 left-0 w-100 mt-5"
      id="header"
    >
      {/* <TopNav flag={flag}/> */}
      <Navbar expand="lg">
        <Container fluid="xl" className="header-container p-0">
          ...
          <Navbar.Toggle aria-controls="basic-navbar-nav">
            <Hamburger className="nav-toogle-icon" color="#BF9B30" />
          </Navbar.Toggle>
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="nav-container d-flex justify-content-between  w-100">
              <Link
                to="/"
                className="nav-link text-uppercase text-white  position-relative"
              >
                Home
              </Link>
              <Link
              ...
              <Nav.Link>
                <Button
                  variant="warning"
                  className="btn-contact text-uppercase text-white position-relative"
